 ...
 * {@link #onExpiryTriggered(TimerContext)} 中仅投递事件，由业务执行器等完成真正处理。</p>
 *
...
public interface TimerCoordinator<T> {
     ...
     * {@link #onExpiryTriggered(TimerContext)} 返回 {@code false}，同一节点会重新进入等待队列，
     ...
     * {@link #onExpiryTriggered(TimerContext)}。实现通常应同时校验目标状态与
     ...
     */
    boolean onExpiryTriggered(TimerContext<T> context);
}
